...
export function adviceHTML(a){return `<strong>${a.action}${a.amount!==null?' to '+a.amount.toLocaleString('en-US'):a.action==='Call'?' '+a.call.toLocaleString('en-US'):''}</strong><p>${a.reason}</p><p class="advice-metrics">Estimated pot share ≈ ${pct(a.equity)}${a.call?' · Call price '+pct(a.price):' · Checking is free'}</p><details class="estimate-details"><summary>How this estimate works</summary><small>Estimate, not GTO · ${a.trials} simulated runouts against ${a.opponents} random hand${a.opponents===1?'':'s'}. Uses only your cards, the board and public chips. Assumes a showdown with no further bets; opponents’ actual ranges may be stronger.</small></details>`;}
