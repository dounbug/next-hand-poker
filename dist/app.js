...
function render(){
 ...
 $('#decision').innerHTML=`${winnerHTML(review.result)}<div class="next-hand-actions"><button class="primary" data-action="next">Deal next hand <span aria-hidden="true">→</span></button></div>${rankingHTML(review.result,card)}<div class="review compact-review"><div class="review-heading"><h2>Overall play</h2><span class="review-net ${change>=0?'positive':''}">${change>=0?'+':''}${n(change)}</span></div><p><strong>${review.comparison.result}</strong> · You: ${review.hands[0].rank}</p><p>${review.overall}</p><div class="review-sections"><section><h3>What went well</h3><p>${review.wentWell}</p></section><section><h3>Decisions to revisit</h3><p><strong>${review.cue.tone==='good'?'Next question':review.cue.title}</strong></p><p>${review.cue.tone==='good'?review.cue.next:review.cue.body}</p>${review.cue.tone==='good'?'':`<p>${review.cue.next}</p>`}</section></div>${review.opponent?`<p class="opponent-cue">${review.opponent.name}: ${review.opponent.action} on the ${review.opponent.street}, holding ${review.opponent.rank}. These cards were hidden when you acted.</p>`:''}${streetReviewHTML(review.streets,card)}<p class="review-note">Player rows show the revealed hands. Style describes behavior, not a good/bad grade; a strong final hand alone does not prove good play.</p></div>`;
