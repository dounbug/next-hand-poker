...
export function currentHandHTML(hole,board){const h=currentHand(hole,board);if(!h)return '';const suits={s:'♠',h:'♥',d:'♦',c:'♣'};return `<details class="hand-hint"><summary>Your hand now <span>Reveal hint</span></summary><section class="current-hand" aria-label="Your current hand"><div><span class="current-hand-label">YOUR HAND NOW</span><strong>${h.name}</strong><span>${h.detail}${h.boardPlays?' · Board plays':''}</span></div><div class="made-hand-cards" aria-label="${board.length>=3?'Best five cards':'Your cards'}">${h.cards.map(c=>`<span class="made-card ${'hd'.includes(c[1])?'red':''}">${c[0]==='T'?'10':c[0]}${suits[c[1]]}</span>`).join('')}</div></section></details>`;}
