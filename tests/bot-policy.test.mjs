...
test('gentler Omar sometimes just calls a premium opening hand instead of always applying pressure',()=>{const g=createGame();g.actor=4;g.dealer=4;g.players[4].hole=['Ac','Ad'];g.players[4].actedAt=null;assert.equal(botAction(g,()=>.75).type,'call');assert.equal(botAction(g,()=>.2).type,'raise');});
