...
export class Rooms {
 constructor({now=Date.now,onChange=()=>{},saved=[]}={}){this.now=now;this.onChange=onChange;this.rooms=new Map(saved.map(r=>{r.paused=true;r.lastSeen={};r.history??=[];rememberHand(r.history,r.game);return [r.id,r];}));}
 ...
 create(name){if(this.rooms.size>=100)throw Error('Local room limit reached.');const r={id:randomBytes(6).toString('hex'),members:[{seat:0,name:this.name(name),token:secret()}],game:null,history:[],decisions:{},paused:false,ready:[],revision:1,lastSeen:{},created:this.now()};this.rooms.set(r.id,r);this.onChange();return {id:r.id,token:r.members[0].token};}
 ...
 changed(r){r.history??=[];rememberHand(r.history,r.game);r.revision++;this.onChange();}
 ...
 else if(input.type==='ready'){if(r.game?.street!=='complete')throw Error('Finish this hand first.');if(input.hand!==r.game.hand)throw Error('This hand has already changed. Refresh your table.');rememberHand(r.history??=[],r.game);this.start(r);}
 ...
 const result={id:r.id,seat:m.seat,host:m.seat===0,revision:r.revision,paused:r.paused,ready:[...r.ready],members,history:structuredClone(r.history||[]),game:null,review:null};if(!g)return result;
